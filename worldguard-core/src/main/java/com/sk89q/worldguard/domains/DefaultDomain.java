    ...
    private Component toPlayersComponent(ProfileCache cache) {
        ...
        final Iterator<TextComponent> profiles = profileMap.keySet().stream().sorted().map(name -> {
            ...
            if (uuid == null) {
                return TextComponent.of(name, TextColor.YELLOW)
                        .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Только имена", TextColor.GRAY)
        ...
        if (!uuids.isEmpty()) {
            builder.append(TextComponent.of(uuids.size() + " неизвестный uuid" + (uuids.size() == 1 ? "" : "s"), TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Невозможно определить имя для:", TextColor.GRAY)
