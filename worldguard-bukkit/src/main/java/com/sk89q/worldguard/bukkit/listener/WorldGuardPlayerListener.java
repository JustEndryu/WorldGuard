...
public class WorldGuardPlayerListener extends AbstractListener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (cfg.activityHaltToggle) {
            ...
            if (removed > 10) {
                log.info("Интенсивная активность: " + removed + " сущностей (>10) автоматически удалено в мире "
                        + player.getWorld());
            }
        }
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if (wcfg.useRegions && !WorldGuard.getInstance().getPlatform().getSessionManager().hasBypass(localPlayer, localPlayer.getWorld())) {
            ...
            if (!test.apply(event.getMessage())) {
                ...
                RegionProtectionListener.formatAndSendDenyMessage("используй " + event.getMessage(), localPlayer, message);
                event.setCancelled(true);
                return;
            }
        }
